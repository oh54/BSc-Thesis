...
public class ChangeDetectionRGB {
	...
	public static void main(String[] args) throws Exception {
		...
		JavaPairRDD<String, PortableDataStream> data1 = files1
				.mapToPair(new PairFunction<Tuple2<String, PortableDataStream>, String, PortableDataStream>() {
					@Override
					public Tuple2<String, PortableDataStream> call(Tuple2<String, PortableDataStream> t) throws Exception {
		...
		JavaPairRDD<String, PortableDataStream> data2 = files2
				.mapToPair(new PairFunction<Tuple2<String, PortableDataStream>, String, PortableDataStream>() {
					@Override
					public Tuple2<String, PortableDataStream> call(Tuple2<String, PortableDataStream> t) throws Exception {
						String[] tokens = t._1.split("/");
		...
		JavaPairRDD<Text, PngImageWritable> proccessedImgs = joinedData
				.mapToPair(new PairFunction<Tuple2<String, Tuple2<PortableDataStream, PortableDataStream>>, Text, PngImageWritable>() {
					@Override
					public Tuple2<Text, PngImageWritable> call(Tuple2<String, Tuple2<PortableDataStream, PortableDataStream>> t) throws Exception {
		 ...
		 * { String[] tokens = t._1.split("/");; String fileName =
		 * tokens[tokens.length - 1]; //BufferedImage origImg =
		 * ImageIO.read(t._2.open()); return new Tuple2(fileName, t._2); });
		 * 
		 ...
		 * ImageIO.read(t._2.open()); return new Tuple2(fileName, t._2); });
		 */
		// JavaPairRDD<String, Tuple2<PortableDataStream, PortableDataStream>>
		// joinedData = data1.join(data2);

		/*
		 * JavaPairRDD<Text, PngImageWritable> proccessedImgs =
		 * joinedData.mapToPair(t -> { BufferedImage img1 =
		 * ImageIO.read(t._2._1.open()); BufferedImage img2 =
		 * ImageIO.read(t._2._2.open());
		 * 
		 * BufferedImage processedImg = new BufferedImage(img1.getWidth(),
		 * img1.getHeight(), BufferedImage.TYPE_INT_RGB);
		 * 
		 * for (int x = 0; x < processedImg.getWidth(); x++) { for (int y = 0; y
		 * < processedImg.getHeight(); y++) { int r1 = (new Color(img1.getRGB(x,
		 * y))).getRed(); int g2 = (new Color(img2.getRGB(x, y))).getGreen();
		 * processedImg.setRGB(x, y, (new Color(r1, g2, (r1 + g2) /
		 * 2).getRGB())); } }
		 * 
		 * return new Tuple2(new Text(t._1), new
		 * PngImageWritable(processedImg)); });
		 */
